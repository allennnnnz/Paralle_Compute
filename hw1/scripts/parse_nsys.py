...
def plot_runtime_breakdown(df: pl.DataFrame, out_path: Path, case: str, dpi: int = 150) -> None:
    ...
    ax.set_xlabel("# of processes (12 cores/node)")
    ax.set_ylabel("runtime (seconds)")
    ax.set_title(f"Runtime breakdown by processes – case {case}")
    ax.set_xticks(x)
    ax.legend()
...
def plot_speedup(df: pl.DataFrame, out_path: Path, case: str, dpi: int = 150) -> None:
    ...
    ax.set_xlabel("# processes (12 cores/node)")
    ax.set_ylabel("speedup")
    ax.set_title(f"Speedup vs processes – case {case}")
    ax.set_xticks(x)
